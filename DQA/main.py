...
if __name__ == '__main__':
    ...
    for index, data in loop_save:
        if query_times > 0:
            image_name = query_set_path + '/' + data  # 拼接图像的读取地址r'\{}\\'.format(item)
    ...
    for index, data in loop_detective:
        if detecitve_times > 0:
            image_name = detecitve_file_path + '/' + data  # 拼接图像的读取地址r'\{}\\'.format(item)
            image_PIL = Image.open(image_name).convert('RGB')
            image_save = transform_train(image_PIL)
            image_list.append(image_save)
    ...
    if args.method == 'CIP':
        output_list = poison_fun_tensor(Victim_model,image_tensor,None,args.attack,args.ratio,args.dataset,args.trigger_path,upside_down,poison = args.poison)  # TODO:
