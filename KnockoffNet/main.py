...
# ------------------------------------------------------------------------
# KnockoffNets: Stealing Functionality of Black-Box Models
# Orekondy, Tribhuvanesh and Schiele, Bernt and Fritz, Mario
# paper: https://arxiv.org/abs/1812.02766
# codes: https://github.com/tribhuvanesh/knockoffnets
# -----------------------------------------------------------------------
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KNOCKOFF',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--attack', type=str, default='KNOCKOFF', choices=['KNOCKOFF', 'DFME'],help='choosing the attack forms')
    parser.add_argument('--dataset', type=str, default='CIFAR10', choices=['CIFAR10', 'CIFAR100', 'FOOD101'], help='choosing the dataset for target model')
    parser.add_argument('--target_model', type=str, default='wrn28', choices=['resnet18', 'wrn28'],help='choosing the model for target model')
    parser.add_argument('--Surrogate_model', type=str, default='wrn28', choices=['resnet18', 'wrn28'],help='choosing the model for target model')
    ...
    parser.add_argument('--epoch', type=int, default=100, help='setting the epoch for target model training')
    parser.add_argument('--lr', type=float, default=0.1, help='setting the learning rate for target model training')
    parser.add_argument('--batch', type=int, default=128, help='setting the batch_size for target model training')
    parser.add_argument('--num_workers', type=int, default=4, help='setting the num_worker for target model training')
    parser.add_argument('--query_set', type=str, default='./dataset/Imagenet_100K', help='setting the query data')
    ...
    parser.add_argument('--ratio', type=float, default=17, help='setting the poison ratio')
    ...
    '''setting the target model and Surrogate model'''
    ...
    if args.method == 'CIP':
        output_list = poison_fun_tensor(args, target_model, image_tensor, None, args.attack, args.ratio, args.dataset,
                                        args.trigger_path, SPPD, poison=args.poison)
    elif args.method == 'DP':
        output_list = DP_poison(target_model, image_tensor, None, poison=args.poison)
    elif args.method == 'DAWN':
        output_list = DAWN_poison(target_model, image_tensor, None, args.attack, args.ratio, args.dataset,
                                  args.trigger_path, poison=args.poison)
