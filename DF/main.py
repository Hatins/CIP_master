...
def train(args, Victim_model, Surrogate_model,Generator,optimizer,epoch,device):
    ...
    for i in range(args.epoch_itrs):
        ...
        for _ in range(args.d_iter):
            ...
            # with torch.no_grad():
            #     t_logit = Victim_model(fake)

            if args.loss == "l1" and args.no_logits:
                t_logit,energy= get_ood_scores_single(fake, Victim_model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack', type=str, default='Datafree',help='setting the attack form')
    parser.add_argument('--Surrogate_model',type=str,default='resnet18',choices=['wrn_28','resnet18'],help='setting the surrogate model')
    ...
    parser.add_argument('--Victim_model',type=str,default='resnet18',choices=['wrn','resnet18'],help='setting the Victim model')
    parser.add_argument('--pt_file',type=str,default='/home/heyujie/wxy_project/CIP/Victim_Model_Train/Trained/Datafree/Mnist_resnet18_epoch_64_accuracy_99.61%.pt',help='setting the Victim model')
    ...
    Victim_model.load_state_dict(torch.load(args.pt_file,map_location=device))
    Victim_model.eval()
    Victim_model = Victim_model.to(device)

    print('the victim is from '+ args.pt_file)
    print('the Victim model is ' + args.Victim_model)
    ...
    print('Victim - Test set: Accuracy: {}/{} ({:.4f}%)'.format(correct_number, len(test_loader.dataset),accuracy))
    ...
    elif args.Surrogate_model == 'wrn_28':
        Surrogate_model = WideResNet(depth=28, num_classes=args.num_classes, widen_factor=10, dropRate=0.3)
    elif args.Surrogate_model == 'Lenet':
        Surrogate_model = lenet(num_classes=args.num_classes)
    ...
    for epoch in range(1, number_epochs + 1):
        ...
        energy_list = train(args, Victim_model=Victim_model, Surrogate_model=Surrogate_model,Generator=Generator,
    ...
    writer = pd.ExcelWriter(args.dataset+'.xlsx')
    information_dataframe.to_excel(writer, args.dataset, float_format='%.2f')
    writer.close()
